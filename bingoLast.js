...
const calcBoardAndIndex = idx => {
 ...
 for (let i = 0; i < winningIndecesArrays.length; i++) {
  ...
  for (let j = 0; j < winningIndecesArrays[i].length; j++) {
    ...
    if (tempArr.includes(winningIndex)) {
     count++
     if (count === 5) {
      if (!winningBoard.includes(board)) {
      winningBoard.push(board)
      winningIndexOfNumCalled = idx;
      lastBoard = winningBoard[winningBoard.length - 1]
      }
     }
    }
  }
 ...
 return lastBoard, winningIndexOfNumCalled;
}

const getSums = (arr, lastBoard, inputNums) => {
 ...
 let winningCard = [];
 let beginningIndex = lastBoard * 25;
 for (let i = 0; i < 25; i++) {
...
const checkNums = (arr, inputNums) => {
 ...
 return getSums(arr, lastBoard, inputNums)
}
